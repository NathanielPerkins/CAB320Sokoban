...
def tabooCells(puzzleFileName):
    ...
    taboo = []
    for (x,y) in walls:
        if (x+1,y+1) in walls and (x,y) not in walls:
            taboo.append((x,y))
        elif (x-1,y+1) in walls and (x,y) not in walls:
            taboo.append((x,y))
        elif (x-1,y-1) in walls and (x,y) not in walls:
            taboo.append((x,y))
        elif (x+1,y-1) in walls and (x,y) not in walls:
...
if __name__ == "__main__":
    ...
    puzzleFileName = "warehouses/warehouse_01.txt"
    myWare = cab320_sokoban.Warehouse()
    myWare.read_warehouse_file(puzzleFileName)
    test = myWare.visualize()
    print (1,1) in myWare.walls
    taboo = tabooCells(puzzleFileName)
    print taboo
    #lines = tabooCells(puzzleFileName)
    #print lines
    #workers = extract_locations(lines)
    #print workers
